...
标准库零依赖，不 import 图引擎：写入只经 skill "loo0ng-graph" 的 scripts/graph.py 子进程，它仍是图的唯一写入口。
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--engine", default=None, help="图引擎 graph.py 的路径，默认取兄弟 skill loo0ng-graph 里的")
