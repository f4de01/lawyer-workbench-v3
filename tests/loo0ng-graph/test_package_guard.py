...
运行：python -m unittest tests/loo0ng-graph/test_package_guard.py
...
两条判据取或，与 skill "loo0ng-setup-case" 的 setup.py 那份同源（那边只报不拒）：
  ①（真包）按 graph.py 自己的位置算出的兄弟 skill assets/，装在哪儿都成立
  ②（假包）目录名摆成 <...>/loo0ng-domain/assets/<领域名>，别处拷来的一份包
...
真包 = REPO / "skills" / "loo0ng-domain" / "assets"
...
class Base(unittest.TestCase):
    def setUp(self):
        ...
        # 判据 ②：一份摆成 <...>/loo0ng-domain/assets/<领域名> 的假包
        self.假种子 = self.tmp / "loo0ng-domain" / "assets" / "菜园"
...
class 写包内即拒Case(Base):
    ...
    def test_拒绝消息告诉律师往哪走(self):
        ...
        for 片 in ("活图", "sketch.py home", "AGENTS.md", "loo0ng-setup-case"):
...
class 判据Case(Base):
    ...
    def test_判据二认别处拷来的一份包(self):
        self.assertTrue(graph.in_package(self.种子图), "形如 loo0ng-domain/assets/<领域名> 的没被认出")

    def test_活图不被误判(self):
        self.assertFalse(graph.in_package(self.活图), "包外的活图被误判成种子了")

    def test_名字像但形状不对的不误判(self):
        """只有 assets/ 的上一级恰是 loo0ng-domain 才算：别的 skill 的 assets/ 不是种子。"""
        别的 = self.tmp / "loo0ng-to-docx" / "assets" / "菜园"
