...
运行：python -m unittest tests/loo0ng-to-docx/test_layout_estimate.py
...
import support
from support import FIXTURES, convert, template
...
class LayoutEstimateCase(unittest.TestCase):
    ...
    def build(self, case):
        ...
        return support.rewrite(out, self.dir / ("%s-%d-空段.docx" % (name, case["空段"])),
...
class EstimateCoversTheMeasuredTruth(LayoutEstimateCase):
    ...
    def test_the_blank_page_pair_is_exactly_what_the_third_grade_is_for(self):
        ...
        bands = []
        for case in [c for c in FROZEN if c["件"] == "推算-空白页页边界.md"]:
