...
class 入口表Test(unittest.TestCase):
    ...
    def test_显示名是中文且与工作区指针块一致(self):
        指针块 = (REPO / "skills" / "loo0ng-setup-case" / "references" / "工作区AGENTS.md").read_text(encoding="utf-8")
    ...
    def test_登记进了插件与README(self):
        ...
        self.assertIn("./skills/ask-loo0ng", skills)
        self.assertIn("`ask-loo0ng`（问路）", (REPO / "README.md").read_text(encoding="utf-8"))
