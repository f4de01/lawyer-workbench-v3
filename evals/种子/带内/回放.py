...
行高钉 = '<w:trPr><w:trHeight w:hRule="exact" w:val="4000"/></w:trPr>'


def 钉住第一行行高(path: pathlib.Path) -> int:
    """把 path 那件 docx 的第一张表第一行行高钉成固定 200 磅，原地换掉，返回钉了几行。"""
    ...
    with zipfile.ZipFile(tmp, "w", zipfile.ZIP_DEFLATED) as zo:
        for item, data in items:
            if item.filename == "word/document.xml":
                ...
                xml, done = re.subn(r"(<w:tr(?: [^>]*)?>)", r"\1" + 行高钉, xml, count=1)
