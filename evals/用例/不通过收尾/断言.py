...
编排 skill "loo0ng-doit" 拿到门禁退出码 1（不通过）之后做什么，以及做到第 3 次仍不过时收不收得住
...
与人工触发看着，别为了断这个数去改转换器的落点。Claude Code 侧拼成 /loo0ng-doit <提示词>，Codex 
侧用替身提示词，测的是正文不是触发。Codex 侧用 danger-full-access：沙箱里 PATH 上没有 python（#28）。
实测 Claude Code 侧 32 至 40 回合、Codex 侧 7 至 10 回合（Claude 侧五次、Codex 侧三次），上限取 
70，余量按 #26 的教训留够。票里预计「回合数明显高过普通的出一版」，实测不是：32 至 40 与「办节点
出一版」的 37 至 43 在同一档，原因见下。还有一条比次数更要紧的局限，是照着一次真回复看出来的：这
