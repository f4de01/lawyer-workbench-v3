...
def _基线(workspace):
    return json.loads((workspace / "基线.json").read_text(encoding="utf-8"))
...
def check_领域图恰好多出那一个节点(workspace, reply):
    ...
    基线 = _基线(workspace)
    assert len(_nodes(data)) == 基线["领域图节点数"] + 1, \
        "领域图该只多出一个节点（回流前 %d 个），实际 %d 个" % (基线["领域图节点数"], len(_nodes(data)))
    assert len(data["模块"]) == 基线["领域图模块数"], "模块「%s」已在领域图里，不该多出模块" % 模块
...
def check_只生成没拍板的那个没回流(workspace, reply):
    ...
    assert not 带过去的, "只生成没拍板的节点不是候选（判据 a，ADR-0012），却进了领域图：%s" % 带过去的
...
def check_雏形文件没落进工作区(workspace, reply):
    ...
    assert names == ["基线.json", "案件", "领域图.json"], \
