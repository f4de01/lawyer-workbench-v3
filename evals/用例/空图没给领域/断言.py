...
# 「把这一问抛回给律师」的痕迹。不认问号：实测模型问得很清楚却一个问号都不打
# （「请给一个领域名，两种情形都行：」）。真正兜住「没问就往下走」的是上面三条写没写盘的断言，
# 这一条只要求回复里确实在要这个名字。
要领域名 = r"[？?]|请(给|说|告诉)|告诉我|给一个|你(来)?(给|定|说)|哪个领域|哪一个领域|领域名"


def check_问了领域是哪个(workspace, reply):
    assert "领域" in reply, "回复里一个「领域」字都没有，这一问没问出来：\n%s" % reply
    assert re.search(要领域名, reply), "回复里看不出在要领域名：领域名只能由律师给，不是报一句就往下走：\n%s" % reply
