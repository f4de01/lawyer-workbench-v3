"""四个逐节点回流用例共用的读法：活图在哪、回流之前长什么样、跑完变了什么（#91 验收，ADR-0019）。

活图不在工作区里（它住 `<活图家>/领域/<领域名>/`，跑器每次运行另建一个活图家经 LOO0NG_HOME
交给 harness），断言的签名又只有 (workspace, reply)，所以路径与回流前的指纹由种子
「逐节点回流」写进工作区根的点开头文件 `.活图基线.json`，这里从那份基线里读。
...
# 种子「逐节点回流」摆出来的两个已生成未确认的节点。种子的回放与 tests/loo0ng-domain/test_seeds.py
