...
def preflight(根):
    ...
    if 跑脚本(根, "sync-plugin-version.py", "--check") != 0:
        坏.append("版本三处不齐：先跑 python scripts/sync-plugin-version.py")
    单子 = 待结算的(根)
    if not 单子:
...
def postflight(根, 上一版):
    ...
    节 = 抽一节((根 / "CHANGELOG.md").read_text(encoding="utf-8"), 新)
    if not 节:
...
def main(argv=None):
    ...
    ap.add_argument("动作", choices=["preflight", "postflight", "notes"])
    ...
    if args.动作 == "notes":
        ...
        号 = args.指定版本 or 版本(根)
        节 = 抽一节((根 / "CHANGELOG.md").read_text(encoding="utf-8"), 号)
        if not 节:
