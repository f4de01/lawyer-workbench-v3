...
│                        #   回放助手.py 六个路由种子共用的起手与写图动作；基线.py 三份图文件的 sha256（路由只读的判据）；
│                        #   路由断言.py ask-matt 五条验收项加「只指向表里的三个入口」，七个路由用例各 import 一遍
...
合入门槛 = 脚本层全绿 + 两侧 eval 全绿 + 三条校验；关票门槛 = Codex 与 Claude Code 各真实触发一次（ADR-0015）。

全绿只有一件例外：Codex 侧的 `出雏形` 列为已知抖动，单件红放行（律师 2026-09-08 裁定，#68；见 ADR-0015 的同日附注）。只对这一件、只在 Codex 侧、且只在其余 19 件全绿时成立；别的用例红仍是阻塞，PR 里要点名这件红并写明与本票无关。
...
种子挑与要验的路径最接近的那个：`在办中` 是办到一半（12 模块 72 节点、一份已确认一份待确认），`空目录` 用来验起手，`图引擎` 是合成小领域「菜园」。
